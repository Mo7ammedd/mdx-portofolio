...
export const metadata = generateBlogPostSEO({
  ...
  description: 'How OFFSET and cursor pagination use B-tree indexes, how their performance differs, and what happens when data changes between pages.',
...
Use OFFSET when readers need to jump to a page in a small or stable dataset. For deep, sequential browsing, use a cursor with a unique sort order and a matching index.

</Callout>

<span id="what-is-pagination-really" className="block" aria-hidden="true" />
...
```sql title="offset-pagination.sql" {5,11,17} showLineNumbers
...
At page 1 you're reading 10 rows. At page 1,000 (with page size 10), you're reading 10,000 rows to return 10. At page 10,000, you're reading 100,000 rows to return 10. The database does proportionally more work for every deeper page, even though the client receives the same 10 rows each time.
...
```sql title="cursor-pagination.sql" {10-11} showLineNumbers
...
### Try the Pagination Scan Model

Change the page depth and compare the index entries each strategy visits for one request. This model assumes the cursor for that page is already available from a previous response.
...
A <Term id="b-tree">B-tree</Term> index is a balanced tree structure. The leaf nodes are doubly-linked and contain the actual sorted key values. Every lookup starts at the root and descends through internal nodes using binary comparisons until reaching the correct leaf.
...
Descend from the root to the correct leaf using the cursor value: **O(log n)**.

</Step>
<Step title="Read the next page">

Read the next `LIMIT` rows from that position: **O(limit)**.

</Step>
<Step title="Continue from the last row">

Return the page and use its final row as the cursor for the next request.
...
A <Term id="composite-cursor">composite cursor</Term> records every ordering value needed to identify a unique position.
...
If multiple rows share the same `created_at` timestamp (common with bulk inserts), rows at the boundary can be skipped. Use a **composite cursor** that includes a unique tiebreaker, typically the primary key, in both the filter and the sort order.
...
SQL Server does not support tuple inequality syntax such as `(created_at, id) < (@created_at, @id)`. Expand the cursor predicate to `created_at < @created_at OR (created_at = @created_at AND id < @id)`, with a matching index and ordering. SQL Server's `OFFSET ... ROWS FETCH NEXT ... ROWS ONLY` syntax still pays the cost of skipping the offset rows.


## Concurrency and Snapshot Isolation

A cursor records an ordering position; it does **not** preserve a database snapshot. <Term id="mvcc">MVCC</Term> and the transaction isolation level determine which row versions a query can see. Page requests in separate transactions can therefore observe different committed data, even when each request uses repeatable read or snapshot isolation.

With a unique, immutable sort order, keyset pagination avoids shifting its boundary when rows before the cursor are inserted or deleted. Each query still sees the rows visible to its own transaction. New rows after the boundary may appear on a later page, and changing a row's ordering values can cause it to be skipped or returned again.

For an export that must represent one consistent point in time, use a database-supported consistent snapshot spanning the operation, or materialize the result set. A long-lived snapshot has its own database and resource costs; putting a cursor in an HTTP response does not create one.
...
<Exercise id="pagination-delete" />

## Verified Examples

Checked on **September 16, 2026** with **PostgreSQL 18.6**. The SQL check used 103 rows with tied timestamps and 10-row pages. It compared OFFSET and composite-cursor results across all 11 pages, including the final partial page and an exhausted cursor, and confirmed that a timestamp-only cursor can skip tied rows.

The reproducible check is `scripts/verify-blog/pagination.sql` in the portfolio repository. It verifies query results and page boundaries; the interactive model above illustrates index entry counts under its stated assumptions.
