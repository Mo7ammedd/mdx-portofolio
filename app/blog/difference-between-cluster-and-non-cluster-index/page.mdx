...
export const metadata = generateBlogPostSEO({
  title: 'Clustered vs Non Clustered Database Indexes',
  description: 'How clustered and non-clustered indexes store data, how B-trees work, and what that means for reads and writes.',
...
Most relational databases use <Term id="b-tree">B-tree</Term> structures for indexes. This includes SQL Server, PostgreSQL, MySQL (InnoDB), and Oracle.
...
A <Term id="covering-index">covering index</Term> includes the columns this query needs and can eliminate the key lookup.
...
<span id="smart-maintenance-script" className="block" aria-hidden="true" />
