...
Implementation and deployment notes for [modev.me](https://www.modev.me).
See the [README](../README.md) for an overview and local setup.
...
Review questions and publish replies at the private `/ask/inbox`.
...
development. Run `node scripts/setup-ask.mjs --email you@example.com` to configure
the local inbox email and generate a password.
See [Q&A setup](ask-setup.md) for the database migration, environment
...
The Writing page (`/blog`) lists article titles, descriptions, publication dates,
and estimated reading times. Descriptions come from each article's metadata.
Reading paths, saved articles, and the glossary are linked below the list.
Full-text search is available from the reading controls inside an article.
...
See [the blog authoring guide](blog-authoring.md) for code filenames, line
...
checks support freshness labels; see [the verification record](verification/2026-09-16.md).
